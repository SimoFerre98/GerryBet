...
export default function LoginPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (!email.trim()) {
      setMessage("L'email è obbligatoria per la registrazione.")
      return
    }
  ...
  const isSuccess =
    message.toLowerCase().includes('controlla') ||
    message.toLowerCase().includes('successo')
    ...
    <div className="flex flex-col items-center justify-center min-h-[100dvh] p-6 bg-transparent">
      <div className="max-w-md w-full bg-white/40 backdrop-blur-xl border border-white/60 p-10 rounded-[3rem] shadow-2xl shadow-indigo-200/50">
        ...
        <div className="text-center mb-8">
          <Link
            href="/"
            className="inline-block mx-auto w-20 h-20 mb-4 transform transition hover:scale-105 drop-shadow-2xl"
          >
            <img
              src="/logo.jpg"
              alt="GerryBet Logo"
              className="w-full h-full object-contain rounded-full border-2 border-indigo-400/30 shadow-lg shadow-indigo-500/20"
            />
          </Link>
          <h2 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-900 to-indigo-600 tracking-tight">
            GerryBet
          </h2>
        </div>

        {/* Tab switcher */}
        <div className="flex rounded-2xl overflow-hidden border border-white/60 mb-7 bg-white/30">
          <button
            id="tab-login"
            type="button"
            onClick={() => { setTab('login'); resetMessage() }}
          ...
          <button
            id="tab-register"
            type="button"
            onClick={() => { setTab('register'); resetMessage() }}
            className={`flex-1 py-3 text-sm font-bold transition-all duration-300 ${
              tab === 'register'
                ? 'bg-indigo-600 text-white shadow-md'
                : 'text-indigo-800 hover:bg-white/40'
        ...
        {tab === 'login' && (
          <form className="space-y-5" onSubmit={handleSignIn}>
            <div>
              <label htmlFor="login-email" className="block text-sm font-bold text-indigo-900 mb-1 ml-1">
            ...
            <div>
              <div className="flex justify-between items-center mb-1 ml-1">
                <label htmlFor="login-password" className="block text-sm font-bold text-indigo-900">
        ...
        {tab === 'register' && (
          <form className="space-y-5" onSubmit={handleSignUp}>
            <div>
              <label htmlFor="reg-username" className="block text-sm font-bold text-indigo-900 mb-1 ml-1">
            ...
            <div>
              <label htmlFor="reg-email" className="block text-sm font-bold text-indigo-900 mb-1 ml-1">
                Email <span className="text-red-500">*</span>
              </label>
              <input
                id="reg-email"
            ...
            <div>
              <label htmlFor="reg-password" className="block text-sm font-bold text-indigo-900 mb-1 ml-1">
                Password <span className="text-red-500">*</span>
              </label>
              <div className="relative">
                <input
